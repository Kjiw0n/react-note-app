...
test("renders Note component with initial elements", () => {
...
test("saves a new note to localStorage", () => {
  (useParams as jest.Mock).mockReturnValue({ id: undefined });
  renderWithTheme(<Note isDarkMode={false} toggleTheme={mockToggleTheme} />);

  fireEvent.change(screen.getByPlaceholderText(/노트 제목을 입력해주세요./i), {
    target: { value: "Test Title" },
  });
  fireEvent.change(screen.getByPlaceholderText(/노트 내용을 입력해주세요./i), {
...
test("loads and updates an existing note", () => {
  const mockNote = {
    id: "1",
    title: "Existing Note",
